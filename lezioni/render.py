...
if __name__ == '__main__':
    # mic
    folder = sys.argv[1]
    shutil.copy('aferrari.css', '../' + folder + '/theme/css/aferrari.css')
    shutil.copy('slide_config.js', '../' + folder + '/slide_config.js')
    for filename in os.listdir(folder):
        if filename.endswith(".md"):
            filename = filename[:-3]
            inpath = folder + '/' + filename + '.md'
            outpath = '../' + folder + '/' + filename + '.html'
